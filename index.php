...
$stmt = $pdo->prepare('select dp.DETAIL_ID,SUBSTR(dp.PATIENT_DETS,1,120) as PATIENT_DETS,
tp.PATIENT_NAME, tp.PATIENT_SURNAME, tp.PATIENT_ID, tp.PATIENT_NAME, tp.PATIENT_SURNAME,
tp.PATIENT_AGE from DET_PATIENTS dp join TBL_PATIENTS tp
on (dp.PATIENT_ID = tp.PATIENT_ID)');
  ...
  <body class="bg-gray-100">
    ...
    <section>
      <div class="container mx-auto p-4 mt-4">
        <div class="text-center text-3xl mb-4 font-bold border border-gray-300 p-3">Recent Patients</div>
        <div class="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
<?php foreach($users as $user) : ?>
          <!-- Job Listing: <?= $user['PATIENT_ID'] ?> -->
          <div class="rounded-lg shadow-md bg-white">
            <div class="p-4">
              <h2 class="text-xl font-semibold"><?= $user['PATIENT_NAME'] ?></h2>
